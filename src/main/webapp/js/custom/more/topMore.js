...
var rTopMore = {
		...
		listRes : function(result)  {
			...
			for(var i=0; i<cnt; i++) {
				rTopMore.addListRow(result.pageData.list[i], parseInt(result.pageData.currentPage, 10), i);
			}
		...
		addListRow : function (dataObject, page, cnt) {
			...
			innerHtml += '</tr>';
			$('#topSongDiv tbody').append(innerHtml); 
		}
};
